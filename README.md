...
* `pulse-point-htb.js` - This is your partner module file, by default it contains a template divided into multiple sections which need to be completed.
* `pulse-point-htb-validator.js` - This is the validator file for the configuration object that will be passed into your module.
* `pulse-point-htb-exports.js` - A file that contains all of the modules exports (i.e. any functions that need to be exposed to the outside world).
...
1. <b>Complete the pulse-point-htb.js file </b>
    * pulse-point-htb.js is where all of your adapter code will live.
...
2. <b>Complete the pulse-point-htb-validator.js file</b>
...
3. <b>Complete the pulse-point-htb-exports.js file</b>
    * This file will contain any functions that need to be exported or exposed to the outside world. Things like render functions, custom callbacks, etc. Any legacy render functions will also need to be exposed here. Anything added to the `shellInterface.PulsePointHtb` will be accessible through `window.headertag.PulsePointHtb`
...
for (var i = 0; i < 10; i++) {
    ...
    if (w.headertag) {
        try {
            w.headertag.PulsePointHtb.render(document, %%PATTERN:TARGETINGMAP%%, '%%WIDTH%%', '%%HEIGHT%%');
            break;
        } catch (e) {
            continue;
        }
...
In this section you will be filling out the pulse-point-htb.js, pulse-point-htb-exports.js, and the pulse-point-htb-validator.js files to create your module.

### Step 0: Config Validation (`pulse-point-htb-validator.js`)
...
Once you have a basic idea of what this will look like, and how you will uniquely identify each slot on your server (via xSlot placementId or other inventory codes) you will need to validate this configuration. This validation will be performed by the wrapper using the `pulse-point-htb-validator.js` file.

The `pulse-point-htb-validator.js` file contains a single export, a `partnerValidator` function, that takes in the configuration object that will be fed to your module's constructor (refer to [Configuration](#configuration) for an example layout) and validates it via type checks. The type checks are performed using an external library called `schema-inspector`, for which the documentation can be found here https://github.com/Atinux/schema-inspector.

We have provided a very basic validation schema in `pulse-point-htb-validator.js` that is based off of the example `mockPartnerConfig.js` object found in the `spec/support` directory for testing (refer to the [Testing](#testing) section for the testing structure).

Once you have filled this file out, you can continue actually writing your module!

### Step 1: Partner Configuration (`pulse-point-htb.js`)
...
### Step 2: Generate Request URL (`pulse-point-htb.js`)
...
### Step 3: Response Callback (`pulse-point-htb.js`)
...
### Step 4: Parsing and Storing Demand (`pulse-point-htb.js`)
...
### Step 5: Rendering Pixel (`pulse-point-htb.js`)
...
### Step 6: Exports (`pulse-point-htb-exports.js`)
...
```javascript
shellInterface.PulsePointHtb = { //shell interface is the window variable that is accessable through the window object, currently this will always be window.headertag
...
```javascript
if (__directInterface.Layers.PartnersLayer.Partners.PulsePointHtb) {
    shellInterface.PulsePointHtb = shellInterface.PulsePointHtb || {};
    shellInterface.PulsePointHtb.adResponseCallback = __directInterface.Layers.PartnersLayer.Partners.PulsePointHtb.adResponseCallback;
}
```
...
if (__directInterface.Layers.PartnersLayer.Partners.PulsePointHtb) {
    shellInterface.PulsePointHtb = shellInterface.PulsePointHtb || {};
    shellInterface.PulsePointHtb.adResponseCallbacks = __directInterface.Layers.PartnersLayer.Partners.PulsePointHtb.adResponseCallbacks;
}
```
